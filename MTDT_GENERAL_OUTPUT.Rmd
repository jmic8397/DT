...
dataDir = "./melanomaAssaysNorm.RData"
WDir = "C:/Users/jamie/Desktop/UniY3S1/DataScience/DENR3901/Prereadings/NewMelanomaData"
...
ssercutoffList= list(c(0.4, 0.4, 0.4))
...
#May require a list of different param Objects for each Experiment
...
for(cutoff in 1:length(ssercutoffList)){
  ...
  MTDTresults = GeneralMTDT(MultiAssayExperiment,
                        tierList,fixedTier,ssercutoffList[[cutoff]],
  ...
  for(nperm in 1:nperms){
    ...
    for(i in 1:length(df$levelName)){
      ...
      print(df$counterSum[i])
        g = g + geom_point(aes_(x=((MTDTsummary$TSER.summary$TSA.retained[nperm] * MTDTsummary$TSER.summary$N.retained[nperm] ) + (MTDTsummary$TSER.summary$TSA.notretained[nperm] * MTDTsummary$TSER.summary$N.notretained[nperm]))/MTDTsummary$TSER.summary$N.Total[nperm] , y= MTDTsummary$TSER.summary$Cost.Total[nperm], size = df$counterSum[i]/df$counterSum[length(df$counterSum)], colour = df$counterSum[i] ),alpha = 0.05, show.legend=FALSE) 
    }
  }

totalTierAccuracies = ((MTDTsummary$TSER.summary$TSA.retained * MTDTsummary$TSER.summary$N.retained ) + (MTDTsummary$TSER.summary$TSA.notretained * MTDTsummary$TSER.summary$N.notretained))/MTDTsummary$TSER.summary$N.Total
...
g = g+ geom_text(label = MTDTsummary$TSER.summary$Tier.Sequence, hjust=0, vjust=0)
 g = g +  scale_size(range = c(5,30)) + coord_cartesian(xlim= c(0.9*min(totalTierAccuracies),1.1*max(totalTierAccuracies)), ylim=c(0.9*min(MTDTsummary$TSER.summary$Cost.Total),1.1*max(MTDTsummary$TSER.summary$Cost.Total)))
print(g)
